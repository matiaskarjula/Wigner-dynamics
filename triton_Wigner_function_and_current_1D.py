...
# collapse operators
c_op_list = []
c_op_list.append(np.sqrt(kappa * (1 + n_th_a)) * a)  # decay operators
c_op_list.append(np.sqrt(kappa*n_th_a) * a.dag())  # excitation operators
...
# Hamiltonians
H_0 = a.dag() * a + .5                                             
H_D = (1-omega_D)*(a.dag() * a + .5) + (Omega_hamiltonian/2)*(a + a.dag())
...
# Function that calculates the Wigner function and corresponding current
    ...
    for U in U_values:
        for Omega in Omega_values:
            ...
            for i in range(1,s_res-1,factor):  # row, column indexing is easier in python 
                for j in range(1,s_res-1,factor):
                    J_sys_xhat[i,j] = ((1-omega_D-U)*pvec[j] + ((U*pvec[j]**3)/(2)) + ((U*pvec[j]*xvec[i]**2)/(2)))*W.T[i,j] - ((U*pvec[j])/(8))*W2X[i,j] - ((U*pvec[j])/(8))*W2P[i,j]
                    J_sys_phat[i,j] = ((omega_D+U-1)*xvec[i] - ((U*xvec[i]**3)/(2)) - ((U*xvec[i]*pvec[j]**2)/(2)) - ((Omega)/(np.sqrt(2))))*W.T[i,j] + ((U*xvec[i])/(8))*W2P[i,j] - ((U*xvec[i])/(8))*W2X[i,j]
                    J_env_xhat[i,j] = -kappa/2 * ((n_th_a + 0.5)*WX[i,j] + xvec[i]*W.T[i,j]) 
                    J_env_phat[i,j] = -kappa/2 * ((n_th_a + 0.5)*WP[i,j] + pvec[j]*W.T[i,j])
            ...
            namef = f"coherent_1D_U{U}_Omega{Omega}_{str(r).zfill(5)}.npz"
...
def makePlots(polku):
    ...
    for p in Path(polku).iterdir():
        ...
        namef = f"coherent_1D_U{U}_Omega{Omega}_{str(r).zfill(5)}.png"
...
#             filename = f"coherent_1D_U{U}_Omega{Omega}_{str(r).zfill(5)}.png"
...
if __name__ == "__main__":
    ...
    #for computetime in computetimes:
    #compute_wigner(computetime, dir=parsed.folder)

    computetimes = list(range(1, 5, plotinterval))
    for computetime_i, computetime in enumerate(computetimes):
        print(f"{computetime_i}/{len(computetimes)}")
        compute_wigner(computetime, dir=parsed.folder)

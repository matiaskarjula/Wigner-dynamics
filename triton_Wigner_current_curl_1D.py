...
def compute_wigner(r, dir):
    for U in U_values:
        for Omega in Omega_values:
            ...
            WPXX = np.zeros((s_res, s_res))
            WXPP = np.zeros((s_res, s_res))
            ...
            for i in range(1,s_res-1,factor):  # Implicitly enforces Dirichlet boundary condition    
                    for j in range(1,s_res-1,factor):
                        ...
                        WPXX[i, j] = (WP[i+1, j] - 2*WP[i, j] + WP[i-1, j]) / (((range_h-range_l) / (s_res-1))**2)
                        WXPP[i, j] = (WX[i, j+1] - 2*WX[i, j] + WX[i, j-1]) / (((range_h-range_l) / (s_res-1))**2)

            curl_J = 2*(omega_D - 1 + U*(1- (xvec[i]**2) - (pvec[j]**2)))*W.T[i,j] 
            + ((omega_D+U-1)*xvec[i] - ((U*xvec[i]**3)/2) - ((U*xvec[i]*pvec[j]**2)/2))*WX[i,j]
            - ((1-omega_D-U)*pvec[j] + ((U*pvec[j]**3)/2) + ((pvec[j]*xvec[i]**2)/2))*WP[i,j]
            + (U/4)*W2P[i,j] + ((U*xvec[i])/8)*WXPP[i,j] + (U/4)*W2X[i,j] + ((U*xvec[i])/8)*W3X[i,j]
            + ((U*pvec[j])/8)*WPXX[i,j] + ((U*pvec[j])/8)*W3P[i,j]
            + (kappa/2)*xvec[i]*WP[i,j] - (kappa/2)*pvec[j]*WX[i,j] 
            
            
            p = Path(dir)
            p.mkdir(exist_ok=True, parents=True)
            namef = f"curl_1D_U{U}_Omega{Omega}_{str(r).zfill(5)}.npz"
...
def makePlots(polku):
    ...
    for p in Path(polku).iterdir():
        ...
        namef = f"curl_1D_U{U}_Omega{Omega}_{str(r).zfill(5)}.png"
